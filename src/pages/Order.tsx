...
import { onlyDigits, isValidDocument, isValidEmailStrict } from '@/lib/validators'
...
const orderSchema = z.object({
  ...
  email: z.string().trim().email('Email inválido').refine(isValidEmailStrict, 'Email inválido'),
  phone: z.string().refine(val => onlyDigits(val).length === 11, 'Telefone inválido'),
  document: z.string().refine(val => isValidDocument(val), 'CPF inválido'),
...
const Order = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const optionalsSanitized = (configuration.optionals as unknown as string[]).filter(
